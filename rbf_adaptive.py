...
def skew_mesh(a,b,Ns):
    meshes = []
    for N in Ns:
        tmp_mesh = np.linspace(0,1,N)
        tmp_mesh = [x**2 for x in tmp_mesh]
        rev = tmp_mesh[::-1]
        mirror = [2-x for x in rev[1:]]
        tmp_mesh = np.concatenate((tmp_mesh, mirror))
        mesh = [(b-a)* x / 2 + a for x in tmp_mesh]
        meshes.append(mesh)
    return meshes
...
# 1-dimensional galerkin or collocation approximator rewritten to take advantage of overlapping centers
class EfficientApproximator1D:
    def __init__(self, a, b, f, Ns, delta, outer, bc_type='neumann', app_type='galerkin', F=0):
        self.a = a
        self.b = b
        self.f = f
        self.Ns = Ns
        ...
        self.largeN = max(Ns)
        ctrs = self.meshes[-1]
        integrations = np.zeros(shape=(self.largeN, self.largeN))
        s = time.time()
        for i in range(self.largeN):
            xi = ctrs[i]
            for j in range(i + 1):
                xj = ctrs[j]
                ...
                integrations[i,j] = integral
                integrations[j,i] = integral
        e = time.time()
        self.elapsed = e-s
    ...
    def A_(self, level):
        ...
        ratio = (self.largeN - 1) // (N - 1)

        row = 0
        for i in range(N):
            col = 0
            for j in range(N):
                A[i,j] = self.integrations[row,col]
                col += ratio
            row += ratio
    ...
    def innerlevel(self, centers, deltas, alphas, u, neval):
        ...
        for level in range(len(self.Ns)):
            ...
            l_inf_val = np.format_float_scientific(self.l_inf(np.array(app), np.array(solution)), precision=5)
            l_2_val = np.format_float_scientific(self.l_2(uk, u), precision=5)
            lh_2_val = np.format_float_scientific(self.lh_2(uk, u), precision=5)
    ...
    def multilevel(self, u, neval):
        ...
        print(f'Initialization Time: {self.elapsed}')
        print(f'Elapsed Time: {end-start+self.elapsed} seconds')

        # error table
        err_table(np.arange(1, self.outer+1), self.Ns, l_infs, l_2s, lh_2s)

# 1-dimensional galerkin or collocation approximator meant to time the run-time duration of multilevel algorithm
class TimedApproximator1D:
    ...
    def multilevel(self, u, neval, cn_plot=False):
        centers, deltas, alphas = [], [], []
        l_infs, l_2s, lh_2s, cns = [], [], [], []
...
# 1-dimensional galerkin or collocation approximator using adaptive meshes that skews towards boundary
class AdaptiveApproximator1D:
    def __init__(self, a, b, f, Ns, delta, outer, bc_type='neumann', app_type='galerkin', F=0, scale_delta=False):
        self.a = a
        self.b = b
        self.f = f
        self.meshes = skew_mesh(a, b, Ns)
        self.Ns = [len(mesh) for mesh in self.meshes]
        self.outer = outer
        self.bc_type = bc_type
        self.app_type = app_type
        sigma = 3 # floor(sigma) > 2 + d/2 (in 1-d we need sigma > 2.5) 
        if F == 0:
            ...
            self.F = F
        if scale_delta:
            self.deltas = [(delta * (b-a)/(2*N))**(1-(2/sigma)) for N in Ns]
        else:
            self.deltas = [delta for mesh in self.meshes]
